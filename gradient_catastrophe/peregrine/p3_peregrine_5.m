...
% iu_t+u_{xx}+2|u|^2u=0.
  ...
  for nn=1:nmax                        % integration begins
    ...
    if mod(nn,round(nmax/25)) == 0
       ...
       RMSE=sqrt(sum(u(:)-anal(:)).^2/numel(x));
  ...
  fname = ['peregrine_' num2str(N) 'pts_256plotted_' 'point0001eps'];
  ...
  fname = ['peregrine_' num2str(N) 'pts_512plotted' 'point0001eps'];
  saveas(1,fname,'fig');       % save figure
  saveas(1,fname,'png');
  ...
  fname = ['peregrine_' num2str(N) 'pts_1024plotted' 'point0001eps'];
  saveas(1,fname,'fig');       % save figure
  saveas(1,fname,'png');
